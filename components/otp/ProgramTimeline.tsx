...
// Types for OTP Program matching the page
interface MonthData {
    plan: number
    actual: number
    wpts_id?: string
    ...
    const epg = useMemo(() => {
        ...
        programs.forEach(program => {
            MONTHS.forEach((month, monthIndex) => {
                ...
                const isBehind = monthData.actual < monthData.plan && monthData.plan > 0
                const isComplete = monthData.actual >= monthData.plan && monthData.plan > 0

                events.push({
                    id: `${program.id}-${month}`,
                    channelUuid: String(program.id),
                    title: `P:${monthData.plan} A:${monthData.actual}`,
                    description: monthData.pic_name || program.plan_type || "",
                    since: startDate.toISOString(),
                    till: endDate.toISOString(),
                    image: "",
                    color: isBehind ? "#ef4444" : isComplete ? "#10b981" : "#f59e0b"
                })
            })
        })
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: programData.color || "#3b82f6",
                                    borderRadius: "8px",
                                    padding: "4px 8px",
                                    overflow: "hidden",
                                    cursor: "pointer",
